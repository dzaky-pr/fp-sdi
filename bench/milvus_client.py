...
def connect(retries=60, wait=1.0):
    ...
    for _ in range(retries):
        try:
            ...
            pass
        time.sleep(wait)
    # coba open koneksi hingga berhasil
    last=None
    for _ in range(retries):
        try:
            c = MilvusClient(uri="http://milvus:19530")
            c.list_collections()  # sanity
            return c
...
def build_index_ivf(client, n, metric, nlist):
    client.create_index("bench","vec",{
        "index_type":"IVF_FLAT","metric_type":metric,"params":{"nlist": int(nlist)}
    })

def build_index_hnsw(client, metric, M, efc):
    client.create_index("bench","vec",{
        "index_type":"HNSW","metric_type":metric,"params":{"M":int(M),"efConstruction":int(efc)}
    })

def build_index_diskann(client, metric):
    client.create_index("bench","vec",{
        "index_type":"DISKANN","metric_type":metric,"params":{}
    })
...
def run_build(client, kind, n, dim, metric, idx_cfg):
    drop_recreate(client, dim, metric)
    builders = {
        "ivf": lambda: build_index_ivf(client, n, metric, idx_cfg.get("nlist") or _nlist_rule(n)),
        "hnsw": lambda: build_index_hnsw(client, metric, idx_cfg["M"], idx_cfg["efConstruction"]),
        "diskann": lambda: build_index_diskann(client, metric),
    }
    builders[kind]()
